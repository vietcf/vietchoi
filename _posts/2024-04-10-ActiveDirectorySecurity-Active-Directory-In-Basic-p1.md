...
**Bước 2.** Khi một người dùng muốn kết nối đến Service trên mạng như Share Folder, website or database. Họ sẽ sử dụng TGT của mình để yêu cầu KDC cấp một Ticket Granting Service (TGS). TGS là các Ticket chỉ cho phép kết nối đến Service cụ thể (phục vụ cho việc truy cập các Service này). Để yêu cầu một TGS, người dùng sẽ gửi username của họ và timestamp (dấu thời gian) được mã hóa bằng Session Key, cùng với TGT và một Service Principal Name (SPN), chỉ ra service và server name chúng ta dự định truy cập.

Như kết quả, KDC sẽ gửi cho chúng ta một TGS cùng với một Service Session Key, mà chúng ta sẽ cần để xác thực đến Service chúng ta muốn truy cập. TGS được encrypted bằng cách sử dụng một Key tạo ra từ Service Owner Hash. Service Owner ở đây có thể là User account hoặc Machine account (Với Account ở đây có nghĩa là bao gồm username cả password) mà dịch vụ chạy dưới. TGS chứa một bản sao của Service Session Key trong nội dung đã encrypted của nó để  Service Owner có thể truy cập vào nó bằng decrypt TGS.

![Keberos step 2]( {{site.url}}/assets/img/2024/04/10/07-keberos-step2.png)

**Bước 3.** TGS sau đó có thể được gửi đến Service mong muốn sử dụng để xác thực và thiết lập kết nối. Service sẽ sử dụng Key tạo ra từ Service Owner Hash của chính mình sở hữu để giải mã TGS và xác minh Service Session Key hợp lệ.

![Keberos step 3]( {{site.url}}/assets/img/2024/04/10/07-keberos-step3.png)

Đến là có khả năng sử dụng dịch vụ rồi <=> quá trình xác thực Kerberos kết thúc ^^
